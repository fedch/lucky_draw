...
# Display all restaurants history record:
@app.route('/history')
def history():
    # Get a list of history from the database in descending order by created time. Limit number of records to 20:
...
@app.route('/top')
def top():
    # Also descending order of draw by adding a minus sign. No minus sign would mean ascending order of draw
    restaurants = Restaurants.query.order_by(text('-draw')).limit(5)
    return render_template('top.html', nav='top', restaurants=restaurants)


# Instead of the current time display a meal time:
